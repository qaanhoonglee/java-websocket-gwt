...
public class ChatView extends Composite implements WebSocketListener {
    ...
    private void connectWebSocket() {
        try {
            ...
            webSocketManager.setCurrentUserName(userName);
            ...
            addMessage("Hệ thống", "Đang kết nối tới: " + wsUrl + " với tên người dùng: " + userName);
    ...
    private void sendMessage() {
        ...
        try {
            ...
            // Gửi tin nhắn thông qua WebSocketManager
            webSocketManager.sendChatMessage(message, recipient);
    ...
    // Implement các phương thức của WebSocketListener
            ...
            if (message.startsWith("{")) {
                ...
                if (message.contains("\"type\"")) {
                    ...
                    if ("chat".equals(type)) {
                        ...
                        String recipient = WebSocketManager.extractJsonField(message, "recipient");
                        String content = WebSocketManager.extractJsonField(message, "content");
                        addMessage("[Đã gửi riêng cho " + recipient + "]", content);
                        return;
                    } else if ("system".equals(type)) {
                        String systemMsg = WebSocketManager.extractJsonField(message, "message");
                        addMessage("Hệ thống", systemMsg);
                        return;
                    } else if ("error".equals(type)) {
                        String errorMsg = WebSocketManager.extractJsonField(message, "message");
                        addMessage("Lỗi", errorMsg);
                        return;
                    } else if ("welcome".equals(type)) {
                        String welcomeMsg = WebSocketManager.extractJsonField(message, "message");
                        addMessage("Server", welcomeMsg);
                        return;
                    } else if ("register-confirm".equals(type)) {
                        String userName = WebSocketManager.extractJsonField(message, "userName");
